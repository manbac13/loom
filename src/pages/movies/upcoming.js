...
const UpcomingMovies = () => {
  ...
  const debouncedSearch = useCallback(
    debounce((search) => {
      setIsTransitioning(true);
      getSearchedMovieAction({ query: search, for: "upcomingMovies" });
    }, 600),
    []
  );
  ...
  useEffect(() => {
    if (movieSearched) {
      ...
      getUpcomingMoviesAction(paramsForFetch);
    }
  }, [paramsForFetch, movieSearched]);

  useEffect(() => {
    if (!loading && upcomingMovies) {
      const timer = setTimeout(() => {
        setDisplayData(upcomingMovies?.results);
